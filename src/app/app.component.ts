...
export class AppComponent {
  ...
  randomNum() {
    return Math.floor((Math.random() * (this.maxNum4D - this.minNum4D + 1)) + this.minNum4D);
  }
  ...
  alertUserToto() {
    if(this.numTotoCount === 5) {
      alert("Draws take place every Monday and Thursday at 6.30pm. If it is a cascade draw, the draw time will be at 9.30pm.")
  ...
  generateTotoNum() {
    this.huatNumToto = [];
    for(let i =0; i < 6; i++) {
      this.tempTotoNum = Math.floor((Math.random() * (this.maxNumToto - this.minNumToto+ 1)) + this.minNumToto);
  ...
  quotes: string[] = ["A Gambler is nothing but a man who makes his living out of false hope. - William Bolitho",
